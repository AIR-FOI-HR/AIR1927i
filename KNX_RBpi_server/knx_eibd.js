...
function groupsocketlisten(opts, callback) {
  ...
  conn.openGroupSocket(0, function(parser) {
     ...
     if(src != "1.1.5"){
       console.log('Write from ' + src + ' to ' + dest + ': ' + val +' DPT:' + dpt);
...
// This function reads values on an addres from bus
// ReadFromBus("0/2/40", callback);
function readFromBus(dest, callback) {
  var opts = { host: "localhost", port: 6720 }
    , conn = new eibd.Connection();
...
readFromBus('1/1/1', function(data){
...
// exports.ReadFromBus = readFromBus;
